...
void omp_cumsum(long int* x, long int n){
    ...
    for (long int cur=2; cur<=n; cur*=2){
    	...
    	//Q: is it possible to move parallel region creation outside the critical for loop?
    	#pragma omp parallel private(i), shared(n, prev, cur, x)
    	{
...
void omp_extract_if(const std::vector<T>& in, std::vector<T>& out, Pred pred) {
	...
	#pragma omp parallel shared(flags, in, cums), private(i)
	{
	#pragma omp for
